"""
Adapted from https://github.com/clovaai/rainbow-memory/blob/master/methods/finetune.py
...
class Finetune:
    def __init__(
        ...
        # if pretrained is enable, it will keep params from task t-1
        # to be used in task t
        self.pretrain = kwargs["pretrain"]

        # use dataset root path if given, used when training on different machine (cluster)
        self.dataset_root = kwargs["dataset_root"]
    ...
    def before_task(self, datalist, cur_iter, init_model=False, init_opt=True):
        ...
        # incoming class different from each task
        # some openloris dataset does not have class in some segments,
        # makesure it always have 121 classes for domain incremental
        if "openloris" in self.exp_name:
            # cls is CIL setting
            if "disjoint" in self.exp_name:
                incoming_classes = pd.DataFrame(datalist)["klass"].unique().tolist()

            else:
                # force incoming classes to 121
        ...
        logger.info(f"Number of seen classes (old+new): {len(self.exposed_classes)}")
        logger.info(f"Number of currently-learning classes: {self.num_learning_class}")
        ...
        if init_model:
            # do not reset model if self.pretrained is enabled (fine tuning model from task t-1)
            # todo: remove this fucntion in naive (redundant impl)
            if not self.pretrain:
                ...
                logger.info(f"Using model from previous task in pretrain mode")

                # if disjoint, expand the fc layer to accommodate new classes
                if "disjoint" in self.dataset:
                    logger.info(f"Expand fc layer for new classes")
    ...
    def after_task(self, cur_iter):
        ...
        if self.mem_manage is None:
            logger.warning("mem_manage is None, no memory is replayed")
        else:
    ...
    def train(
        ...
        for epoch in range(n_epoch):
            ...
            if eval_after_epoch:
                ...
                writer.add_scalar(f"task{cur_iter}/test/loss", eval_dict["loss"], epoch)
                writer.add_scalar(f"task{cur_iter}/test/acc", eval_dict["acc"], epoch)

                logger.info(
                    f"Task {cur_iter} | Epoch {epoch+1}/{n_epoch} | train_loss {train_loss:.4f} | train_acc {train_acc:.4f} | "
                    f"test_loss {eval_dict['loss']:.4f} | test_acc {eval_dict['acc']:.4f} | "
                    f"lr {self.optimizer.param_groups[0]['lr']:.4f}"
                )

                best_acc = max(best_acc, eval_dict["acc"])
    ...
    def evaluation(self, test_loader, criterion):
        ...
        ret = {"loss": avg_loss, "acc": avg_acc, "cls_acc": cls_acc}
    ...
    def equal_class_sampling(self, samples, num_class):
        ...
        ret = []
        for y in range(self.num_learning_class):
            cls_df = sample_df.loc[sample_df["label"] == y]
            ret += cls_df.sample(n=min(mem_per_cls, len(cls_df))).to_dict(
                orient="records"
            )
        num_rest_slots = self.memory_size - len(ret)
